 ...
 * @date 2021/3/30 7:39
 */
public class ImplicitCPS implements ArithmeticInterf {
    @Override
    public boolean run() {
        ...
        for (int i = 0; i < data.SIZE; i++) {
            ...
            for (int j = 0; j < data.SIZE; ) {
                ...
                for (int k = j; k < data.SIZE; ) {
                    k = Utils.getCandidateKey(map, k, 2);
                    ...
                    if (hsCells.size() == 2) {
                        int[] pvs = new int[]{j, k};
                        final boolean b = Utils.holdSpecifyCellsPV(new Cell[]{cells1.get(0), cells1.get(1)}, pvs);
                        if (b) {
